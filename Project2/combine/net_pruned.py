...
class Net_pruned(nn.Module):
    def __init__(self, input_nc=3, output_nc=3, ngf=64, norm_layer=nn.InstanceNorm2d, n_blocks=6, gpu_ids=[]):
        ...
        # Pruning residual blocks from n_blocks to 3
        # Original loop:
        # for i in range(n_blocks):
        #     model += [block(ngf*expansion, ngf, 1, None, norm_layer)]
        # Manually adding the first 3 blocks:
        if n_blocks >= 1: model += [block(ngf*expansion, ngf, 1, None, norm_layer)] # Block 1
        if n_blocks >= 2: model += [block(ngf*expansion, ngf, 1, None, norm_layer)] # Block 2
        if n_blocks >= 3: model += [block(ngf*expansion, ngf, 1, None, norm_layer)] # Block 3
        # The following blocks are pruned (assuming n_blocks was originally >= 6):
        # if n_blocks >= 4: model += [block(ngf*expansion, ngf, 1, None, norm_layer)] # Block 4 (Pruned)
        # if n_blocks >= 5: model += [block(ngf*expansion, ngf, 1, None, norm_layer)] # Block 5 (Pruned)
        # if n_blocks >= 6: model += [block(ngf*expansion, ngf, 1, None, norm_layer)] # Block 6 (Pruned)
        # Add more commented lines if n_blocks could be > 6 in some cases
